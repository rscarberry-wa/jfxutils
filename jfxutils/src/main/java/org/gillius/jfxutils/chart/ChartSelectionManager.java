...
/**
 * ChartSelectionManager manages a zooming selection rectangle and the bounds of the graph. It can be
 ...
 * </ol>
 * <p>
 * A lot of code in ChartSelectionManager currently assumes there are no scale or rotate
 ...
 * <pre>
 * ChartSelectionManager zoomManager = new ChartSelectionManager( chartPane, selectRect, chart );
 * zoomManager.start();</pre>
 *
 * @author Jason Winnebeck
 * @author Hollis Waite
 */
public class ChartSelectionManager {
    /**
     * The default mouse filter for the {@link ChartSelectionManager} filters events unless only primary
    ...
    private final BooleanProperty mouseDragZoomAllowed = new SimpleBooleanProperty(true);
    ...
    private final Consumer<Rectangle2D> selectionCallback;

    private final Timeline zoomAnimation = new Timeline();

    /**
     * Construct a new ChartSelectionManager. See {@link ChartSelectionManager} documentation for normal usage.
     *
     ...
     */
    public <X, Y> ChartSelectionManager(Pane chartPane,
                                        Rectangle selectRect,
                                        XYChart<X, Y> chart,
                                        Consumer<Rectangle2D> selectionCallback,
                                        boolean mouseDragZoomAllowed,
                                        boolean mouseWheelZoomAllowed) {
        ...
        this.mouseDragZoomAllowed.set(mouseDragZoomAllowed);
        this.mouseWheelZoomAllowed.set(mouseWheelZoomAllowed);
    ...
    /**
     * If true, allow zooming via mouse drag.
     * @return
     */
    public boolean isMouseDragZoomAllowed() {
        return mouseDragZoomAllowed.get();
    }

    /**
     * Get the property on whether or not to allow zooming via mouse dragging.
     * @return
     */
    public BooleanProperty mouseDragZoomAllowedProperty() {
        return mouseDragZoomAllowed;
    }

    /**
     * Set whether or not to enable zooming via a mouse drag.
     * @param b
     */
    public void setMouseDragZoomAllowed(boolean b) {
        mouseDragZoomAllowed.set(b);
    }
    ...
    private void onMouseReleased() {
        ...
        Rectangle2D selectionWindow = chartInfo.getDataCoordinates(
                selectRect.getTranslateX(), selectRect.getTranslateY(),
                rectX.get(), rectY.get()
        );

        if (selectionCallback != null) {
            selectionCallback.accept(selectionWindow);
        }

        if (mouseDragZoomAllowed.get()) {
            ...
            if (zoomAnimated.get()) {
                ...
                zoomAnimation.getKeyFrames().setAll(
                        ...
                        new KeyFrame(Duration.millis(zoomDurationMillis.get()),
                                new KeyValue(xAxisLowerBoundProperty, selectionWindow.getMinX()),
                                new KeyValue(xAxisUpperBoundProperty, selectionWindow.getMaxX()),
                                new KeyValue(yAxisLowerBoundProperty, selectionWindow.getMinY()),
                                new KeyValue(yAxisUpperBoundProperty, selectionWindow.getMaxY())
                        )
                );
                zoomAnimation.play();
            } else {
                zoomAnimation.stop();
                setXAxisLowerBound(selectionWindow.getMinX());
                setXAxisUpperBound(selectionWindow.getMaxX());
                setYAxisLowerBound(selectionWindow.getMinY());
                setYAxisUpperBound(selectionWindow.getMaxY());
            }
        }
    ...
    private static <T> DoubleProperty getLowerBoundProperty(Axis<T> axis) {
        return axis instanceof ValueAxis ?
                ...
                toDoubleProperty(axis, ChartSelectionManager.<T>getProperty(axis, "lowerBoundProperty"));
    }

    private static <T> DoubleProperty getUpperBoundProperty(Axis<T> axis) {
        return axis instanceof ValueAxis ?
                ((ValueAxis<?>) axis).upperBoundProperty() :
                toDoubleProperty(axis, ChartSelectionManager.<T>getProperty(axis, "upperBoundProperty"));
    }
